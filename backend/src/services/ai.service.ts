...
function guessTopic(title: string): string {
  ...
  if (t.includes('network') || t.includes('web') || t.includes('internet') || t.includes('http') || t.includes('osi')) return 'networking';
  if (t.includes('database') || t.includes('sql') || t.includes('mongo') || t.includes('dbms')) return 'databases';
  if (t.includes('math') || t.includes('algebra') || t.includes('calculus') || t.includes('geometry') || t.includes('number')) return 'math';
  if (t.includes('history') || t.includes('war') || t.includes('revolution') || t.includes('empire')) return 'history';
  if (t.includes('programming') || t.includes('javascript') || t.includes('python') || t.includes('code') || t.includes('react')) return 'programming';
  return 'general';
}

const QUESTION_BANK: Record<string, {
  mcq: { text: string; options: string[]; answer: string }[];
  short: string[];
  long: string[];
}> = {
  networking: {
...
export class AIService {
  ...
  private async generateWithGemini(params: {
    ...
    const typeDescription = params.questionTypes.map(t => {
      if (t === 'mcq') return 'Multiple Choice Questions (mcq)';
      if (t === 'short') return 'Short Answer Questions (short)';
      if (t === 'long') return 'Long Answer / Essay Questions (long)';
      return t;
    }).join(', ');
      ...
      properties: {
        sections: {
          ...
          items: {
            type: Type.OBJECT,
            properties: {
              title: { type: Type.STRING, description: "E.g. Section A: Multiple Choice Questions" },
              instruction: { type: Type.STRING, description: "Instructions for this specific section, e.g. Attempt all questions" },
              questions: {
                type: Type.ARRAY,
                items: {
                  type: Type.OBJECT,
  ...
  private generateMockQuestions(params: {
    title: string;
    questionTypes: string[];
    numQuestions: number;
    marksPerQuestion: number;
    ...
    if (activeTypes.length === 0) activeTypes.push('short'); // Fallback
    ...
    const getDiffTag = (idx: number): 'Easy' | 'Moderate' | 'Hard' => {
      if (params.difficulty === 'easy') return 'Easy';
      if (params.difficulty === 'medium') return 'Moderate';
      if (params.difficulty === 'hard') return 'Hard';
      // Mixed: distribute evenly
      if (idx % 3 === 0) return 'Easy';
      if (idx % 3 === 1) return 'Moderate';
      return 'Hard';
    };
    ...
    if (questionsPerType.short > 0) {
      const shortQuestions: IQuestion[] = [];
